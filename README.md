...
# 6. Enrich the seed to Tier 1+ (uses AI Gateway; ~$0.80 for the ~30-entity seed)
...
3. Neon Postgres is the production DB; auto-provisioned through the Vercel Marketplace Neon integration
...
| `pnpm db:push` | Push schema changes directly (local/dev only) |
| `pnpm db:studio` | Open Drizzle Studio |
| `pnpm pipeline:wikidata` | Stream Wikidata JSON dump (use `--stdin` for curl-pipe) |
| `pnpm pipeline:wikipedia` | Stream Wikipedia XML dump into local source text |
| `pnpm pipeline:tags` | Run civilizational tag assignment via `scripts/tag-all.ts` |
| `pnpm pipeline:audit` | Coverage report (entities-per-region-per-era) |
| `pnpm tsx scripts/seed-curated.ts` | Seed the initial ~30 curated entities |
...
| `pnpm tsx scripts/tier3-curate.ts` | Promote the first 5 anchors to Tier 3 |
| `pnpm tsx scripts/tier3-curate-batch-2.ts` | Promote the second 5 anchors to Tier 3 |
...
| `pnpm tsx scripts/promote-from-bulk.ts` | Copy selected QIDs from local bulk DB to production-sized curated DB |
| `pnpm tsx scripts/probe-britannica-coverage.ts` | Persist Britannica articles to `sources` |
| `pnpm tsx scripts/repair-wikipedia-titles.ts` | Retry stuck entities with Wikidata sitelink title fallback |
...
Voyage embeddings cost ~$0.18 / million tokens; the current 376-entity curated corpus embeds for about $0.02. Gemini Flash for narration costs ~$1.50/M input + $9/M output; the full Tier 2 corpus was rewritten for under $5 total.
